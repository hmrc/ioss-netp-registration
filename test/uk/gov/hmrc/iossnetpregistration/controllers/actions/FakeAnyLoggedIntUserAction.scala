...
class FakeAnyLoggedIntUserAction @Inject()(bodyParsers: PlayBodyParsers) extends AnyLoggedInUserAction {

  override def invokeBlock[A](request: Request[A], block: AnyLoggedInUserRequest[A] => Future[Result]): Future[Result] =
    block(AnyLoggedInUserRequest(request, "12345-credId", Enrolments(Set.empty)))
